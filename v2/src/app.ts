...
function viewModel(): Scope {
  ...
  return {
    ...
    // be a fixture. See the note on the live page's copy of this.
    ...
    // Which chain the reader is on. This page is only ever about a *retired*
    // pool, so the claim it has to make is not "nothing here is binding" but
    // "the live pool is elsewhere" -- and on mainnet there is no retired pool
    // at all, which `configured()` above already says.
    network: config.network,
    rehearsalShow: config.network !== "mainnet",
    rehearsalNote: `${config.network} rehearsal · not mainnet money`,
    voteIntro:
      "This floor is the retired vault's. Its proposals were about money this " +
      "contract still holds; nothing new is raised here, and the live pool is " +
      `${config.successor}.`,
    statusNote:
      config.network === "mainnet"
        ? "Mainnet has no retired vault. The live pool is " +
          `${config.successor}, on the main page.`
        : `Retired on ${config.network}. Contracts unaudited. Nothing here is an ` +
          `offer; this page exists so a member can take their money out.`,
